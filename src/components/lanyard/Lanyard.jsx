...
function Band({
  ...
  useFrame((state, delta) => {
    if (dragged) {
      ...
      targetPos.set(vec.x - dragged.x, vec.y - dragged.y, vec.z - dragged.z);
      
      const maxDistance = 6.0; // Max distance card can be pulled from its anchor
      ...
      card.current?.setNextKinematicTranslation({ x: targetPos.x, y: targetPos.y, z: targetPos.z });
    }
    if (fixed.current) {
      [j1, j2].forEach(ref => {
